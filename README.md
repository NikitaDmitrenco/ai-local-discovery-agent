...
User Natural Language / Voice Prompt
   ...
   └──────────────────────────┬─────────────────────────────┘
                              ▼
   ┌────────────────────────────────────────────────────────┐
   │ 3. Multi-Source Discovery (Supabase DB + OSM + Serper) │
   │    • Supabase Cloud PostgreSQL + OpenStreetMap Overpass│
...
### 🗄️ 2. Supabase Cloud PostgreSQL Database
All verified venues, structured attributes, GPS coordinates, operating schedules, authentic photos, and visitor reviews are persisted and indexed in a production **Supabase PostgreSQL** cloud instance with Row Level Security (RLS) and sub-millisecond query performance.

### 🛰️ 3. Real-Time Server-Sent Events (SSE) Streaming
...
### 🗺️ 4. Interactive Geospatial Map with Match Scores
...
### 📅 5. Weekend Trip Itinerary Planner
...
### 🎙️ 6. Web Speech API Voice Search
...
### 💬 7. Conversational Memory & AI Refinement Chips
...
├── lib/
│   └── supabaseClient.ts         # Supabase PostgreSQL client (Anon & Admin Service Role)
├── data/
│   ├── seedSupabase.ts           # Automated Supabase database migration & seeder script
│   └── seedData.ts               # 49+ Verified venue profiles across 40 lifestyle domains
├── components/
...
│   ├── real/
│   │   ├── SupabasePlaceProvider.ts      # Live Supabase PostgreSQL database search provider
│   │   ├── AggregatedPlaceProvider.ts    # Multi-source aggregation (Supabase + OSM Overpass + Serper)
...
│   └── mock/
│       ├── MockPlaceProvider.ts          # Offline fallback provider
│       └── MockLLMProvider.ts            # Deterministic semantic taxonomy engine
├── agent/
